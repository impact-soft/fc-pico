...
//	if( (micros() - demo_time ) > 3 * MICROS_1S ) {
	...
	//--------------------------------
	if ( sys.getKeyTrg() ) {
		if ( sys.getKeyTrg() == TBL_DEBUG_KEY[ debug_cnt ] ) {
...
*/
	...
	if ( (ap.m_timer & 0x7f) == 0x40 ) {
		ap.m_obj[ OBJ_TIT_LOGC ].m_angle_vy = random( -4, 4);
;		ap.m_obj[ OBJ_TIT_LOGC ].m_angle_vx = random( -4, 4);
		ap.m_obj[ OBJ_TIT_LOGC ].m_angle_vz = random( -4, 4);
	}
	...
	c.drawString( "2026 V1.07",8*20, 8*26, _font );
...
#if 0
	{
		...
		sprintf(buffer, "dbcnt:%d", debug_cnt );
