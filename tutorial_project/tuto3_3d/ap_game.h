 ...
 */
enum {
	OBJ_GAM_FIELD = 0,      ///< Unused here; kept so the slot numbering matches the other screens.
	OBJ_GAM_DEBUG = 1,     ///< Debug
	OBJ_GAM_PLAYER = 3,     ///< The player ship.
	OBJ_GAM_BULLET = 4,     ///< First of 15 player-shot slots. // 自機の弾のオブジェ番号
	OBJ_GAM_ENEMY = 48,     ///< First of 22 enemy and enemy slots.
	OBJ_GAM_ENEBLT= 64,     ///< First of 22 enemy and enemy-shot slots.
...
enum {
	...
	PLY_AN_DEAD		= 1,	///< Death animation running. // 死亡アニメ
};
...
//	PLY_DISP_FG		= 0x3B5,	// プレーヤー表示制御
	...
	ENEMY_SUU		= 16, ///< Number of enemy slots.
	
	ENEMY_NT_SUU	= 32, ///< Number of enemy+shot slots.
...
public:
     ...
     *
     */
	int setEnemy( uint8_t kind, float x, float z );
	
	void setEnemyIdx( int idx, uint8_t kind, float x, float z );

	uint8_t m_emobj_kind[ ENEMY_NT_SUU ]; ///< Last kind seen in each enemy slot, so models are rebuilt only on a change.
	int m_emobj_hp[ ENEMY_NT_SUU ];	 ///< 
	int m_emobj_dt[ ENEMY_NT_SUU ];	 ///< 
	uint8_t m_emobj_wp[ ENEMY_NT_SUU ];	 ///< 
	float m_emobj_dir[ ENEMY_NT_SUU ];	 ///< 
	float m_emobj_spd[ ENEMY_NT_SUU ];	 ///< 
	float m_emobj_Targetdir[ ENEMY_NT_SUU ];	 ///< 
	float getEnemySpd( uint8_t spd );

	Obj3d *objPLY;	///< Player OBJ pointer
	Obj3d *objSWD;	///< Sword  OBJ pointer
	Obj3d *objATK;	///< Attack Target OBJ pointer
	Obj3d *objTmp;
...
private:
     ...
     *
     */
	void  shotPlayer( float aofs = 0);
	void  shotPlayerB( float aofs = 0 );
     ...
     *
     */
	bool  moveLimit( Obj3d *obj );
	bool  moveLimitPly( Obj3d *obj );
     ...
     *
     */
	void  setBakuEfc( Obj3d *obj, uint8_t chrno );

    /**
     * @brief 爆発エフェクトアニメーション
     *
     */
	void  anmBakuEfc();
	...
	void _moveSpcSub( int idx, Obj3d *obj );
	void _horming( int idx, Obj3d *obj );
	void setMoveDirData( int idx, Obj3d *obj );

    /**
     * @brief カメラ制御
     *
     * ゲームプレイ中はプレーヤーキャラの後方を追尾するカメラ
     * デモプレイ中はランダムに動くカメラ
     */
	void updateCamera();
     ...
     *
     */
	Obj3d* findNearEnemy( float LimitSQ );
	...
	float ply_dir_x;   ///< Player Move X
	float ply_dir_z;   ///< Player Move Z
	float ply_side_x;   ///< Player Move side X
	float ply_side_z;   ///< Player Move side Z
	float atk_dir;	   ///< attack angle
	float atk_dir_x;   ///< attack Move X
	float atk_dir_z;   ///< attack Move Z
	int DashCounter;	///< Player DashCounter
	...
	uint8_t m_PlyDamFg;
	uint8_t PLY_ANM;
	uint8_t PLY_MUTEKI_TM;
	uint8_t PLY_ATK_CNT;

	rp_mission mt;		// ステージミッション制御
