...
void Canvas::drawLine(int startX, int startY, int endX, int endY) {
	...
	if ( startX <= endX ) {
		if ( endX < clipXL ) return;
		if ( startX > clipXH ) return;
	} else {
		if ( startX < clipXL ) return;
		if ( endX > clipXH ) return;
	}

	if ( startY <= endY ) {
		if ( endY < clipYL ) return;
		if ( startY > clipYH ) return;
	} else {
		if ( startY < clipYL ) return;
		if ( endY > clipYH ) return;
	}
