...
// 光源とベクトルABの内積値から光源値を返す
int getLightData( float cos_sita  ) {
	//※ベクトルの長さが0だと答えが出ませんので注意してください。
	float dt = 0;
...
void polygon_light( GLVertex& A, GLVertex& B, GLVertex& C ) {
	...
	//内積とベクトル長さを使ってcosθを求める
	float cos_sita = dotVector3(light_vector,normal_vector);

//	if ( light_vector[2] < 0.0f ) {
//		cos_sita = -cos_sita;	// 判定条件反転
//	}

   	c.setDitherNo( getLightData( cos_sita ) );
}
...
bool isDispArea( GLVertex *glVertices ) {
	...
	if( glVertices->z >= -1.0 && glVertices->z <= 0.0f)  return true;
	return false;
}
...
//			glCanvas->setSprZoom( 1.0f,1.0f );
        ...
        for(int i = 0; i < glVerticesCount; i++) {
			...
			if(!(glVertices[i].z >= -1.0 && glVertices[i].z <= 1.0)) return;
